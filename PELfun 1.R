...
pel <- function (x,y,info,lambda,v,b.ini,index){
  ...
  while (k <= 5){
    ...
    b.new <- optim(b.old,fb,gb,method = "L-BFGS-B",control = list(maxit=1))$par
    b.new[abs(b.new) <= 0.001] <- 0
    
    # given b, update xi
    fxi <- function(xi){llxi(xi,x,b.new,info,index)}   
    gxi <- function(xi){sxi(xi,x,b.new,info,index)}
    xi.new <- optim(xi.old,fxi,method = "L-BFGS-B",lower=rep(-1/2,K),upper=rep(1/2,K),control = list(maxit=100,fnscale=-1))$par 
